...
!-------------------------------------------------------------------------
!BOP
! \subsection{Fluid Injection Component InjectorMod.F90}
!
! !DESCRIPTION:
...
!EOPI
        ...
        call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_INITIALIZE, userRoutine=injector_init1, phase=1, rc=rc)
        if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
        call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_INITIALIZE, userRoutine=injector_init2, phase=2, rc=rc)
        if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
        call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_RUN, userRoutine=injector_run, rc=rc)
        if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
        call ESMF_GridCompSetEntryPoint(comp, ESMF_METHOD_FINALIZE, userRoutine=injector_final, rc=rc)
        if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
...
!EOE
      ...
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      open(fileunit, status="old", file="./data/coupled_inject_input",action="read",iostat=rc)
      if (rc .ne. 0) then
...
! Give the export state an initial set of values for the SIE Field.
      ...
      call setFieldNeeded(exportState, "SIE", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "U", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "V", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "RHO", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "P", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "Q", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
      call setFieldNeeded(exportState, "FLAG", .false., rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
...
!EOPI
      ...
      do i=1, datacount
         ...
         if (.not. isFieldNeeded(exportState, datanames(i), rc=rc)) then 
             cycle
         endif
